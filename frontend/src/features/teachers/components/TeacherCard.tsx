import { Mail, GraduationCap, Eye, KeyRound } from "lucide-react";
...
type TeacherCardProps = {
  ...
  onView: () => void;
  handleResendOtp: (userId: string, userName: string) => void;
};
      ...
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="font-mono text-xs text-ulead-slate">
              {teacher.employeeId ?? "—"}
            </p>
          </div>
        </div>
      ...
      {teacher.subjects.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5 border-t border-ulead-line pt-3">
          {teacher.subjects.slice(0, 3).map((s) => (
            <span
              key={s.id}
              className="rounded-full bg-marigold/15 px-2 py-0.5 text-xs font-medium text-marigold-deep"
            >
              {s.name}
            </span>
          ))}
